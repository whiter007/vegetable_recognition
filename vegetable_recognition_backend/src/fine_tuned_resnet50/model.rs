...
pub fn fc_layer(
    ...
    Ok(Func::new(move |xs: &Tensor| {
        // 线性层 1
        let linear1 = linear(in_dim, hidden_dim1, vb.pp("0"))?;
        let mut xs = linear1.forward(xs)?;
        // 批量归一化 1
        let bn1 = batch_norm(hidden_dim1, 1e-5, vb.pp("1"))?;
        ...
        // 线性层 2
        let linear2 = linear(hidden_dim1, hidden_dim2, vb.pp("4"))?;
        xs = linear2.forward(&xs)?;
        // 批量归一化 2
        let bn2 = batch_norm(hidden_dim2, 1e-5, vb.pp("5"))?;
        ...
        // 线性层 3
        let linear3 = linear(hidden_dim2, out_dim, vb.pp("8"))?;
